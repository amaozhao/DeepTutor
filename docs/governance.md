...
证据：`web/app/(workspace)/home/[[...sessionId]]/page.tsx` 原约 2193 行，当前约 2014 行；`web/context/UnifiedChatContext.tsx` 原约 1885 行，当前约 1069 行。已把首页 capability/tool catalog 拆到 `web/lib/capabilities.ts`，把附件转换/筛选拆到 `web/lib/attachments.ts`，把持久化 session hydration helper 拆到 `web/lib/chat/hydration.ts`，把聊天 state/reducer 拆到 `web/context/chat/state.ts`，把 runner key 迁移和 stream event 元数据解析拆到 `web/context/chat/transport.ts`；对应测试归位到 `web/tests/lib/capabilities.test.ts`、`web/tests/lib/attachments.test.ts`、`web/tests/lib/chat/hydration.test.ts`、`web/tests/context/chat/state.test.ts` 和 `web/tests/context/chat/transport.test.ts`。

问题：页面、状态管理、网络请求、流式事件和 UI 控制耦合，导致聊天页任何小改都可能影响登录后主流程。

治理动作：已先拆出纯数据 catalog、附件 helper、hydration helper、state/reducer 和部分 transport helper，避免继续扩大聊天页和上下文。下一轮再评估是否继续抽完整 runner hook；纯展示组件、重连和错误提示仍需要更细的回归覆盖。
