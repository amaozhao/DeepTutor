...
5. 已完成：增加备份/恢复文档。
6. 已完成 beta：系统状态已包含 auth/CORS/cookie/PocketBase/多 worker 告警、provider 配置状态、storage/auth/quota/rate store 可写性，并暴露当前文件状态导致 `multi_replica_ready=false`；正式多副本仍需接入外部共享存储后再改为 ready。
