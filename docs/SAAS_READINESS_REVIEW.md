...
`docker-compose.ghcr.yml` 已改为完整挂载 `./data:/app/data`，不会再漏掉 `data/system`、`data/users`、`data/partners`、`data/system/usage`。系统状态已能报告本地 storage、auth store、rate store 和 quota store 是否可写，并明确当前 `multi_replica_ready=false`，阻断原因是 auth/token-version revocation、限流、usage/quota 仍是文件型 beta 状态；其中用户文件写路径和 usage ledger 已有本机文件锁，但仍不是外部共享身份库、外部 token-version 撤销状态源或强一致 quota store。`data/user/settings/shared_state.json` 和 `DEEPTUTOR_SHARED_STATE_PROVIDER` / `DEEPTUTOR_DATABASE_URL` 可以声明计划使用的共享状态来源；即使配置为 `postgres`，当前健康检查仍会把多 worker 标为不安全，直到 auth/rate/quota 真实迁移。启用 PocketBase 时也会额外标记 `pocketbase_multi_user_supported=false`。剩余问题是尚未接入外部共享存储，所以仍不支持多副本一致性。
