...
账号状态和角色变更已经进入认证链路：disabled 用户不能登录，JWT 解码会重新检查用户存在、disabled、user id 和 token version，角色以服务端当前记录为准；改密码、停用、角色变更会 bump token version。用户文件写路径已有本机文件锁和原子替换，普通鉴权读取不加锁，不会把所有用户请求串行化。剩余问题是 token version 仍存在本地用户文件里，只覆盖单机文件型 beta；多副本需要外部共享撤销/session 存储。`logout` 仍只是删除当前浏览器 cookie，没有服务端会话表。
...
`docker-compose.ghcr.yml` 已改为完整挂载 `./data:/app/data`，不会再漏掉 `data/system`、`data/users`、`data/partners`、`data/system/usage`。系统状态已能报告本地 storage、auth store、rate store 和 quota store 是否可写，并明确当前 `multi_replica_ready=false`，阻断原因是 auth/token revocation、限流、usage/quota 仍是文件型 beta 状态；其中用户文件写路径和 usage ledger 已有本机文件锁，但仍不是外部共享身份库或强一致 quota store。启用 PocketBase 时也会额外标记 `pocketbase_multi_user_supported=false`。剩余问题是尚未接入外部共享存储，所以仍不支持多副本一致性。
