...
已加入文件型滑动窗口限流：登录按 IP + username，注册按 IP + email，非 auth 写接口按 IP + principal，WebSocket 认证入口也有限流。当前 bucket 可在同一 `data/system/rate` 挂载下跨本机 worker 共享；剩余问题是它仍不是 Redis/数据库等外部共享存储，不能支撑真正多副本，也没有账号锁定策略、真实 CAPTCHA 服务、风控评分和租户级限流。
...
`docker-compose.ghcr.yml` 已改为完整挂载 `./data:/app/data`，不会再漏掉 `data/system`、`data/users`、`data/partners`、`data/system/usage`。系统状态已能报告本地 storage、rate store 和 quota store 是否可写，并明确当前 `multi_replica_ready=false`，阻断原因是 auth/token revocation、限流、usage/quota 仍是文件型 beta 状态；启用 PocketBase 时也会额外标记 `pocketbase_multi_user_supported=false`。剩余问题是尚未接入外部共享存储，所以仍不支持多 worker / 多副本一致性。
...
- 保持单 FastAPI worker / 单应用副本；如需多副本，先接外部用户库、会话/撤销表和共享 quota 存储。文件型限流只适合受控 beta。
