...
- 支持“可配置开启的邮箱 + 密码普通用户注册”和“管理员一次性邀请码 + 邮箱密码注册”；公开自助注册可配置为先进入 disabled 待审核状态；管理员也可用邮箱密码创建普通用户，或通过 CSV 批量导入普通用户；`auth.max_users` 可做受控 beta 的全局账号上限；支持单副本文件型 LLM turn 用量和 hard quota，TTS/STT/search/embedding 已按调用次数纳入同一额度；支持普通用户自助导出和自助注销的 beta 闭环；不支持邮箱验证、忘记密码、多租户组织等开放 SaaS 产品闭环。账单/支付本轮明确不纳入。
...
- 文件型 usage ledger 支持今日/月度/累计聚合，并用本机文件锁保护追加、读取和保留期清理：`deeptutor/multi_user/usage.py`、`deeptutor/multi_user/data_governance.py`。
...
代码现在会把 LLM turn 的 `cost_summary` 持久化到用户维度 usage ledger，并支持每日/月度 token、调用次数和成本 hard quota。TTS/STT/search/embedding 已按调用次数接入同一 quota ledger；usage ledger 追加、读取和保留期清理已使用本机文件锁，适合单机多 worker beta，但 quota 检查和消费记录仍不是数据库事务。embedding 目前按返回向量数量计调用，不估算真实 token 成本。本轮明确不设计套餐、余额、超额扣费、支付回调、发票或订阅状态。
...
`docker-compose.ghcr.yml` 已改为完整挂载 `./data:/app/data`，不会再漏掉 `data/system`、`data/users`、`data/partners`、`data/system/usage`。系统状态已能报告本地 storage、rate store 和 quota store 是否可写，并明确当前 `multi_replica_ready=false`，阻断原因是 auth/token revocation、限流、usage/quota 仍是文件型 beta 状态；其中 usage ledger 已有本机文件锁，但仍不是外部强一致 quota store。启用 PocketBase 时也会额外标记 `pocketbase_multi_user_supported=false`。剩余问题是尚未接入外部共享存储，所以仍不支持多 worker / 多副本一致性。
...
- 保持单 FastAPI worker / 单应用副本；如需多副本，先接外部用户库、会话/撤销表和强一致 quota 存储。文件型限流和带文件锁的 usage ledger 只适合受控 beta。
