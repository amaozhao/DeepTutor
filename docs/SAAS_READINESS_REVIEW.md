...
账号状态和角色变更已经进入认证链路：disabled 用户不能登录，JWT 解码会重新检查用户存在、disabled、user id 和 token version，角色以服务端当前记录为准；改密码、停用、角色变更会 bump token version。用户文件写路径已有本机文件锁和原子替换，普通鉴权读取不加锁，不会把所有用户请求串行化。剩余问题是 token version 仍存在本地用户文件里，只覆盖单机文件型 beta；多副本需要外部 token-version/session 撤销状态源，保证所有实例共同看到“旧 token 已失效”。这不是给普通鉴权读取加锁，也不等同于现在必须实现完整 session 表。`logout` 仍只是删除当前浏览器 cookie，没有服务端会话表。
...
当前用户文件写路径有本机进程锁和原子替换，可避免同一台机器上的注册、停用、改角色、改密码等写操作互相覆盖；普通鉴权读取不加锁。它仍不是外部共享用户库，跨机器多副本、强一致撤销、会话管理和审计查询都需要数据库或专用身份存储。受控私有部署可以先保持单应用副本；开放 SaaS 不应依赖这个文件锁。
...
`docker-compose.ghcr.yml` 已改为完整挂载 `./data:/app/data`，不会再漏掉 `data/system`、`data/users`、`data/partners`、`data/system/usage`。系统状态已能报告本地 storage、auth store、rate store 和 quota store 是否可写，并明确当前 `multi_replica_ready=false`，阻断原因是 auth/token-version revocation、限流、usage/quota 仍是文件型 beta 状态；其中用户文件写路径和 usage ledger 已有本机文件锁，但仍不是外部共享身份库、外部 token-version 撤销状态源或强一致 quota store。启用 PocketBase 时也会额外标记 `pocketbase_multi_user_supported=false`。剩余问题是尚未接入外部共享存储，所以仍不支持多副本一致性。
...
- 保持单 FastAPI worker / 单应用副本；如需多副本，先接外部用户库、外部 token-version/session 撤销状态源和强一致 quota 存储。文件型限流和带文件锁的 usage ledger 只适合受控 beta。
...
- 多副本部署时仍依赖单机 JSON 里的 `token_version`，没有外部一致的 token-version 撤销状态源来处理用户停用、删除、改密、强制登出或管理员降权。
