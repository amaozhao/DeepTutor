...
本文是对“当前代码是否能直接开放为商业 SaaS”的代码复核结论。范围只包括当前仓库可见逻辑与当前本地配置，不覆盖法律合规、商业定价策略或第三方服务合同。

更新说明：截至 2026-06-30，本仓库已补齐任务规划中的 Milestone 0-4 最小闭环：账号状态会进入鉴权链路，注册方案限定为邮箱 + 密码，不做手机号/SMS/OTP 注册；支持默认关闭的邮箱 + 密码公开注册，也支持管理员创建一次性邮箱注册邀请码；公开注册提交同意时会持久化基础同意记录和当前协议版本，可配置公开自助注册先进入 disabled 待审核状态，增加基础限流、Origin/Referer 防护、附件 session ownership 校验，以及文件型 LLM usage/quota；TTS/STT/search/embedding 也已按调用次数纳入同一 quota ledger。管理员用户操作、停用原因、全局 `auth.max_users` 账号上限、用户清单 CSV 导出/导入、grant、skill 安装、模型目录、邀请码和 MCP 配置写操作已进入审计或创建约束，管理员可查询审计 JSONL；管理员和普通用户均可导出用户数据 zip，普通用户可用当前密码自助注销，删除用户可选择保留、归档或删除 workspace/grant，并可配置基础保留期策略、手动清理过期 audit/usage/deleted-user 数据。PocketBase 已明确保留为单用户集成，启用时会在生产告警和部署状态里标记为不支持多用户/SaaS。下文结论按当前代码状态更新。
...
- 支持“可配置开启的邮箱 + 密码普通用户注册”和“管理员一次性邀请码 + 邮箱密码注册”；公开自助注册可配置为先进入 disabled 待审核状态；管理员也可用邮箱密码创建普通用户，或通过 CSV 批量导入普通用户；`auth.max_users` 可做受控 beta 的全局账号上限；支持单副本文件型 LLM turn 用量和 hard quota，TTS/STT/search/embedding 已按调用次数纳入同一额度；支持普通用户自助导出和自助注销的 beta 闭环；不支持邮箱验证、忘记密码、订阅付费、多租户组织等开放 SaaS 产品闭环。
...
- Cookie 写接口有 Origin/Referer 防护；主要非 auth 写接口有进程内按 IP + principal 的基础限流：`deeptutor/api/main.py:262` 到 `deeptutor/api/main.py:293`。
...
- Turn result 中的 `cost_summary` 会被提取、合并，并在回答保存后记录到 `data/system/usage/llm_usage.jsonl`：`deeptutor/services/session/turn_runtime.py:105` 到 `deeptutor/services/session/turn_runtime.py:130`，`deeptutor/services/session/turn_runtime.py:1692` 到 `deeptutor/services/session/turn_runtime.py:1768`。
- 文件型 usage ledger 支持今日/月度/累计聚合：`deeptutor/multi_user/usage.py:112` 到 `deeptutor/multi_user/usage.py:224`。
...
- 用户数据导出包含 workspace、账号元数据、grant、该用户 usage 和相关 audit；普通用户可在个人资料页自助导出，也可用当前密码注销自己的普通用户账号；删除用户默认保留数据，也可显式归档或删除 workspace/grant：`deeptutor/multi_user/data_governance.py:109` 到 `deeptutor/multi_user/data_governance.py:209`，`deeptutor/api/routers/auth.py:873` 到 `deeptutor/api/routers/auth.py:948`，`deeptutor/api/routers/auth.py:1336` 到 `deeptutor/api/routers/auth.py:1365`。
...
3. Token 撤销仍是单机文件型最小实现

账号状态和角色变更已经进入认证链路：disabled 用户不能登录，JWT 解码会重新检查用户存在、disabled、user id 和 token version，角色以服务端当前记录为准；改密码、停用、角色变更会 bump token version。剩余问题是 token version 存在本地用户文件里，只覆盖单应用副本；多 worker / 多副本需要外部共享撤销/session 存储。`logout` 仍只是删除当前浏览器 cookie，没有服务端会话表。

证据：JWT 生成/校验在 `deeptutor/services/auth.py:251` 到 `deeptutor/services/auth.py:320`；token version bump 在 `deeptutor/multi_user/identity.py:244` 到 `deeptutor/multi_user/identity.py:360`；logout 只删 cookie：`deeptutor/api/routers/auth.py:518` 到 `deeptutor/api/routers/auth.py:522`。

4. 删除用户已有数据策略，但不是完整合规删除

删除用户默认仍保留 `data/users/<uid>`，但管理员删除接口、`/admin/users` 删除确认和普通用户自助注销均已支持 `data_action=keep|archive|delete`：`keep` 保留 workspace/grant，`archive` 移动到 `data/system/deleted_users/`，`delete` 删除 workspace/grant。数据治理设置已能配置 audit/usage/deleted-user 保留天数，并可由管理员手动触发清理。剩余问题是保留期当前没有后台定时任务，也没有法务审批、注销冷静期或恢复流程。
...
5. 有 LLM、语音、search 和 embedding usage/quota，但没有计费、套餐和订阅

代码现在会把 LLM turn 的 `cost_summary` 持久化到用户维度 usage ledger，并支持每日/月度 token、调用次数和成本 hard quota。TTS/STT/search/embedding 已按调用次数接入同一 quota ledger；embedding 目前按返回向量数量计调用，不估算真实 token 成本。剩余缺口是：没有套餐、余额、超额扣费、支付回调、发票、订阅状态或按组织/租户聚合。

证据：usage/quota 实现在 `deeptutor/multi_user/usage.py:112` 到 `deeptutor/multi_user/usage.py:224`；turn runtime 调用在 `deeptutor/services/session/turn_runtime.py:1400` 到 `deeptutor/services/session/turn_runtime.py:1768`；语音路由调用在 `deeptutor/api/routers/voice.py:78` 到 `deeptutor/api/routers/voice.py:156`；search 调用在 `deeptutor/services/search/__init__.py:73` 到 `deeptutor/services/search/__init__.py:193`；embedding 调用在 `deeptutor/services/embedding/client.py:17` 到 `deeptutor/services/embedding/client.py:254`。代码搜索仍没有发现真实支付/账单模块，`stripe|checkout|billing|invoice|payment` 只命中非支付语义的 subscription 文本。

6. 生产级限流和登录保护不足

已加入进程内滑动窗口限流：登录按 IP + username，注册按 IP + email，非 auth 写接口按 IP + principal，WebSocket 认证入口也有限流。剩余问题是这些 bucket 仍在进程内存，不能跨 worker / 多副本共享；也没有账号锁定策略、真实 CAPTCHA 服务、风控评分和租户级限流。

证据：限流器在 `deeptutor/api/security.py:16` 到 `deeptutor/api/security.py:63`；登录/注册限流在 `deeptutor/api/routers/auth.py:562` 到 `deeptutor/api/routers/auth.py:649`；API 写限流在 `deeptutor/api/main.py:262` 到 `deeptutor/api/main.py:293`；WebSocket 连接和 turn 限流在 `deeptutor/api/routers/auth.py:439` 到 `deeptutor/api/routers/auth.py:479`、`deeptutor/api/routers/unified_ws.py:81` 到 `deeptutor/api/routers/unified_ws.py:151`。
...
9. 文件型用户存储不适合多 worker / 多副本直接扩容

当前用户文件写锁只覆盖单进程 FastAPI。多 worker 或多副本会竞态，需要外部共享用户库/数据库和分布式锁。受控私有部署可以先保持单应用副本；开放 SaaS 不应依赖这个文件锁。

证据：`deeptutor/multi_user/identity.py:19` 到 `deeptutor/multi_user/identity.py:24`。
...
TTS/STT/search/embedding 已按调用次数接入用户 quota，但还没有按真实 token/秒数/供应商成本计费。embedding 仍按部署 active profile 解析，不走 per-user embedding model grant。开放 SaaS 要继续把这些共享能力统一纳入精确成本、审计和套餐权限。
...
6. 部署模板和基础健康检查已补，但多副本仍不成立

`docker-compose.ghcr.yml` 已改为完整挂载 `./data:/app/data`，不会再漏掉 `data/system`、`data/users`、`data/partners`、`data/system/usage`。系统状态已能报告本地 storage 和 quota store 是否可写，并明确当前 `multi_replica_ready=false`，阻断原因是 auth/token revocation、限流、usage/quota 仍是文件或进程内状态；启用 PocketBase 时也会额外标记 `pocketbase_multi_user_supported=false`。剩余问题是尚未接入外部共享存储，所以仍不支持多 worker / 多副本一致性。

证据：GHCR compose 挂载：`docker-compose.ghcr.yml:49` 到 `docker-compose.ghcr.yml:52`；健康检查和部署状态字段：`deeptutor/api/routers/system.py:25` 到 `deeptutor/api/routers/system.py:143`。

7. 管理后台能力仍不够 SaaS 化

当前 `/admin/users` 支持邮箱密码创建、删除、改角色、停用/启用、记录停用原因、重置密码、强制登出、分配资源、单用户数据 zip 导出、用户清单 CSV 导出、邮箱密码 CSV 批量导入普通用户、删除数据策略选择、创建一次性注册邀请码，并可在授权编辑器查看今日用量和编辑 quota；`auth.max_users` 可限制全局账号总数；页面也提供最近审计事件查看。普通用户可在 `/profile` 导出自己的数据并用当前密码注销账号。仍缺组织/套餐级席位管理、组织/团队批量导入、批量 grant 分配、导入预览/错误报告等生产级运营能力。
...
当前已有管理员用户数据导出 zip、普通用户自助导出 zip、普通用户当前密码确认注销、删除用户时的 `keep` / `archive` / `delete` workspace/grant 策略、公开注册基础同意记录、协议版本记录、停用原因、基础保留期配置和管理员手动保留期清理。仍缺管理员访问用户数据的审批、区域化存储、后台定时执行的 usage/audit 清理或留存流程、法务文本快照、注销冷静期和恢复流程等完整 SaaS 合规能力。
...
- 保持单 FastAPI worker / 单应用副本；如需多副本，先接外部用户库、会话/撤销表和共享限流存储。
...
- 允许陌生用户自助注册。
- 多副本/多 worker 直接共享当前 JSON 用户文件。
- 多副本部署时仍依赖单机 JSON 里的 `token_version` 来处理用户停用、删除或管理员降权。
- 把 PocketBase 当作已完成的多用户后端。
- 没有配置用户 quota、运营监控和共享限流/共享 quota store 就开放高成本模型。
...
6. 已完成 beta：用户数据导出，以及删除用户时保留/归档/删除 workspace/grant 的策略。
...
11. 已完成 beta：全局 `auth.max_users` 账号上限；正式 SaaS 仍需组织/套餐级席位。
...
- Stripe 或其他支付、套餐、账单、发票、用量扣费。
- 组织/团队/套餐级席位/租户模型；当前只有全局 `auth.max_users`。
- 外部数据库和共享身份存储。
