...
def test_update_carries_a_translatable_template(tmp_path) -> None:
    ...
    """
    ProgressStage = getattr(
        __import__("deeptutor.knowledge.progress_tracker", fromlist=["ProgressStage"]),
        "ProgressStage",
    )
    ProgressTracker = getattr(
        __import__("deeptutor.knowledge.progress_tracker", fromlist=["ProgressTracker"]),
        "ProgressTracker",
    )
