...
def test_mn4_is_connected_but_not_rag_retrievable() -> None:
    ...
    """
    is_connected_kb = getattr(
        __import__("deeptutor.knowledge.kb_types", fromlist=["is_connected_kb"]), "is_connected_kb"
    )
    supports_rag_retrieval = getattr(
        __import__("deeptutor.knowledge.kb_types", fromlist=["supports_rag_retrieval"]),
...
def test_deleting_the_kb_removes_its_synced_store(tmp_path: Path, monkeypatch) -> None:
    ...
    try:
        MarginNoteStore = getattr(
            __import__("deeptutor.capabilities.marginnote4.store", fromlist=["MarginNoteStore"]),
            "MarginNoteStore",
        )
        resolve_db_path = getattr(
            __import__("deeptutor.capabilities.marginnote4.store", fromlist=["resolve_db_path"]),
            "resolve_db_path",
        )
