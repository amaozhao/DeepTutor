...
def test_visualize_pipeline_construction() -> None:
    VisualizePipeline = getattr(
        __import__("deeptutor.agents.visualize.pipeline", fromlist=["VisualizePipeline"]),
        "VisualizePipeline",
    )
...
def test_math_animator_request_config_fields() -> None:
    ...
    MathAnimatorRequestConfig = getattr(
        __import__(
            "deeptutor.agents.math_animator.pipeline", fromlist=["MathAnimatorRequestConfig"]
        ),
...
def test_math_animator_pipeline_construction() -> None:
    pytest.importorskip("deeptutor.agents.math_animator.pipeline")
    MathAnimatorPipeline = getattr(
        __import__("deeptutor.agents.math_animator.pipeline", fromlist=["MathAnimatorPipeline"]),
...
def test_the_book_prompt_loader_contract() -> None:
    ...
    get_book_prompt = getattr(
        __import__("deeptutor.book.blocks._prompts", fromlist=["get_book_prompt"]),
        "get_book_prompt",
    )
    load_book_prompts = getattr(
        __import__("deeptutor.book.blocks._prompts", fromlist=["load_book_prompts"]),
        "load_book_prompts",
    )
