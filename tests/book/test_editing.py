...
def _engine(page: Page) -> BookEngine:
    ...
    BookCompiler = getattr(
        __import__("deeptutor.book.compiler", fromlist=["BookCompiler"]), "BookCompiler"
    )
    CompilerOptions = getattr(
        __import__("deeptutor.book.compiler", fromlist=["CompilerOptions"]), "CompilerOptions"
    )
