...
def test_the_legacy_facade_is_no_longer_imported() -> None:
    ...
    ast = __import__("ast", fromlist=[])
    inspect = __import__("inspect", fromlist=[])

    module = __import__("deeptutor.book.blocks.quiz", fromlist=["*"])
...
def test_the_pipeline_call_is_signature_correct() -> None:
    ...
    QuestionPipeline = getattr(
        __import__("deeptutor.agents.question.pipeline", fromlist=["QuestionPipeline"]),
        "QuestionPipeline",
    )
...
def test_the_context_fields_quiz_sets_all_exist() -> None:
    ...
    UnifiedContext = getattr(
        __import__("deeptutor.core.context", fromlist=["UnifiedContext"]), "UnifiedContext"
    )
