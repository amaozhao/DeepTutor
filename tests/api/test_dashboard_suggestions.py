...
def test_suggestions_is_not_swallowed_by_the_entry_route(
    ...
    service = __import__("deeptutor.services.suggestions", fromlist=["*"])
...
def test_refresh_returns_a_fresh_set(client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    service = __import__("deeptutor.services.suggestions", fromlist=["*"])
    Suggestion = getattr(
        __import__("deeptutor.services.suggestions", fromlist=["Suggestion"]), "Suggestion"
    )
    SuggestionSet = getattr(
        __import__("deeptutor.services.suggestions", fromlist=["SuggestionSet"]), "SuggestionSet"
    )
