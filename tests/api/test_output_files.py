...
def output_app(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> OutputAppFactory:
    auth_router = __import__("deeptutor.api.routers.auth", fromlist=["auth"])
    outputs = __import__("deeptutor.api.routers.outputs", fromlist=["outputs"])
    multi_user_paths = __import__("deeptutor.multi_user.paths", fromlist=["paths"])
