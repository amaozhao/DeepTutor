...
def test_both_readers_of_interface_json_agree_on_a_legacy_file(
    ...
    """
    interface_settings = __import__(
        "deeptutor.services.settings.interface_settings", fromlist=["interface_settings"]
    )
...
async def test_fetch_models_allows_codebuddy_without_base_url(
    ...
    factory_module = __import__("deeptutor.services.llm.factory", fromlist=["factory"])
...
def test_auth_disabled_settings_endpoint_does_not_expose_provider_secrets(
    ...
    Depends = getattr(__import__("fastapi", fromlist=["Depends"]), "Depends")
    ...
    auth_router = getattr(__import__("deeptutor.api.routers", fromlist=["auth"]), "auth")
