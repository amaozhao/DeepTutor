...
def test_delete_session_cleans_only_current_user_artifacts(as_user, monkeypatch) -> None:
    sessions_router = getattr(
        __import__("deeptutor.api.routers", fromlist=["sessions"]), "sessions"
    )
    LearningProgress = getattr(
        __import__("deeptutor.learning.models", fromlist=["LearningProgress"]), "LearningProgress"
    )
    LearningStore = getattr(
        __import__("deeptutor.learning.storage", fromlist=["LearningStore"]), "LearningStore"
    )
    get_attachment_store = getattr(
        __import__(
            "deeptutor.services.storage.attachment_store", fromlist=["get_attachment_store"]
        ),
        "get_attachment_store",
    )
    reset_attachment_store = getattr(
        __import__(
            "deeptutor.services.storage.attachment_store", fromlist=["reset_attachment_store"]
        ),
