...
def bootstrap_hash() -> str:
    hash_password = getattr(
        __import__("deeptutor.services.auth", fromlist=["hash_password"]), "hash_password"
    )
...
def env_admin(mu_isolated_root, monkeypatch, bootstrap_hash):
    ...
    auth_service = getattr(__import__("deeptutor.services", fromlist=["auth"]), "auth")

    monkeypatch.setattr(auth_service, "AUTH_USERNAME", "operator@example.com")
...
def test_first_created_account_is_not_promoted_when_env_admin_exists(env_admin):
    ...
    save_user = getattr(
        __import__("deeptutor.multi_user.identity", fromlist=["save_user"]), "save_user"
    )
...
def test_env_admin_still_resolves_after_first_account_is_created(env_admin, bootstrap_hash):
    load_users = getattr(
        __import__("deeptutor.multi_user.identity", fromlist=["load_users"]), "load_users"
    )
    save_user = getattr(
        __import__("deeptutor.multi_user.identity", fromlist=["save_user"]), "save_user"
    )
...
def test_env_admin_can_log_in_after_first_account_is_created(env_admin):
    ...
    authenticate = getattr(
        __import__("deeptutor.services.auth", fromlist=["authenticate"]), "authenticate"
    )
...
def test_env_admin_is_never_written_into_the_user_store(env_admin):
    ...
    USERS_FILE = getattr(
        __import__("deeptutor.multi_user.identity", fromlist=["USERS_FILE"]), "USERS_FILE"
    )
    save_user = getattr(
        __import__("deeptutor.multi_user.identity", fromlist=["save_user"]), "save_user"
    )
...
def test_env_admin_appears_exactly_once_in_the_admin_user_list(env_admin, bootstrap_hash):
    list_user_info = getattr(
        __import__("deeptutor.multi_user.identity", fromlist=["list_user_info"]), "list_user_info"
    )
    save_user = getattr(
        __import__("deeptutor.multi_user.identity", fromlist=["save_user"]), "save_user"
    )
...
def test_is_first_user_is_false_when_only_the_env_admin_exists(env_admin):
    is_first_user = getattr(
        __import__("deeptutor.services.auth", fromlist=["is_first_user"]), "is_first_user"
    )
...
def bootstrap_client(env_admin, monkeypatch):
    ...
    FastAPI = getattr(__import__("fastapi", fromlist=["FastAPI"]), "FastAPI")
    TestClient = getattr(__import__("fastapi.testclient", fromlist=["TestClient"]), "TestClient")

    auth_router = __import__("deeptutor.api.routers.auth", fromlist=["*"])
    TokenPayload = getattr(
        __import__("deeptutor.services.auth", fromlist=["TokenPayload"]), "TokenPayload"
    )
...
def test_admin_create_user_returns_role_user(bootstrap_client):
    ...
    response = client.post(
        "/api/v1/auth/users",
        json={"username": "student1@example.com", "password": "student-pass-1234"},
        headers=headers,
    )
...
def test_bootstrap_admin_remains_listed_after_creating_an_account(bootstrap_client, env_admin):
    ...
    assert by_name["student1@example.com"]["role"] == "user"
