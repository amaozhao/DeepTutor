...
def mu_isolated_root(tmp_path, monkeypatch) -> Path:
    ...
    """
    grants = getattr(__import__("deeptutor.multi_user", fromlist=["grants"]), "grants")
    identity = getattr(__import__("deeptutor.multi_user", fromlist=["identity"]), "identity")
    paths = getattr(__import__("deeptutor.multi_user", fromlist=["paths"]), "paths")
