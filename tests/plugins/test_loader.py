...
def test_discover_plugins_from_capability_class(monkeypatch):
    loader = getattr(__import__("deeptutor.plugins", fromlist=["loader"]), "loader")
...
def test_load_plugin_capability_skips_tool_entry():
    PluginManifest = getattr(
        __import__("deeptutor.plugins.loader", fromlist=["PluginManifest"]), "PluginManifest"
    )
    load_plugin_capability = getattr(
        __import__("deeptutor.plugins.loader", fromlist=["load_plugin_capability"]),
...
def test_capability_registry_loads_plugins(monkeypatch):
    loader = getattr(__import__("deeptutor.plugins", fromlist=["loader"]), "loader")
    cr = getattr(
        __import__("deeptutor.runtime.registry", fromlist=["capability_registry"]),
        "capability_registry",
    )
