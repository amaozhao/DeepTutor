...
def isolated_settings(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    model_catalog = getattr(
        __import__("deeptutor.services.config", fromlist=["model_catalog"]), "model_catalog"
    )
    runtime_settings = getattr(
        __import__("deeptutor.services.config", fromlist=["runtime_settings"]), "runtime_settings"
    )
    PathService = getattr(
        __import__("deeptutor.services.path_service", fromlist=["PathService"]), "PathService"
    )
    interface_settings = getattr(
        __import__("deeptutor.services.settings", fromlist=["interface_settings"]),
        "interface_settings",
    )
...
def _seed_catalog(tmp_path: Path, *, service: str, profile: str, model: str) -> None:
    ...
    get_model_catalog_service = getattr(
        __import__(
            "deeptutor.services.config.model_catalog", fromlist=["get_model_catalog_service"]
        ),
...
async def test_apply_personal_setting_persists(isolated_settings: Path) -> None:
    get_ui_settings = getattr(
        __import__("deeptutor.services.settings.interface_settings", fromlist=["get_ui_settings"]),
        "get_ui_settings",
    )
...
async def test_apply_rejects_an_engine_that_is_not_installed(
    ...
    """
    get_runtime_settings_service = getattr(
        __import__(
            "deeptutor.services.config.runtime_settings", fromlist=["get_runtime_settings_service"]
        ),
        "get_runtime_settings_service",
    )
    factory = getattr(
        __import__("deeptutor.services.parsing.engines", fromlist=["factory"]), "factory"
    )
...
async def test_a_non_admin_cannot_write_a_global_row(
    ...
    access = getattr(__import__("deeptutor.capabilities.setup", fromlist=["access"]), "access")
...
async def test_a_hung_endpoint_cannot_hang_the_turn(
    ...
    """
    asyncio = __import__("asyncio", fromlist=[])

    _PROBE_TIMEOUT_SECONDS = getattr(
        __import__("deeptutor.services.config.settings_spec", fromlist=["_PROBE_TIMEOUT_SECONDS"]),
        "_PROBE_TIMEOUT_SECONDS",
    )
    _probe_llm = getattr(
        __import__("deeptutor.services.config.settings_spec", fromlist=["_probe_llm"]), "_probe_llm"
    )
    ...
    llm_module = __import__("deeptutor.services.llm", fromlist=["*"])
...
def test_the_first_run_offer_is_not_spent_by_an_ordinary_request(
    ...
    """
    setup_activation = getattr(
        __import__("deeptutor.capabilities.setup.binding", fromlist=["setup_activation"]),
        "setup_activation",
    )
...
def test_concurrent_preference_writes_do_not_lose_updates(isolated_settings: Path) -> None:
    ...
    threading = __import__("threading", fromlist=[])

    _interface_settings_file = getattr(
        __import__(
            "deeptutor.services.settings.interface_settings", fromlist=["_interface_settings_file"]
        ),
        "_interface_settings_file",
    )
    set_ui_setting = getattr(
        __import__("deeptutor.services.settings.interface_settings", fromlist=["set_ui_setting"]),
        "set_ui_setting",
    )
...
async def test_a_silent_job_still_emits_progress(
    ...
    """
    jobs = getattr(__import__("deeptutor.capabilities.setup", fromlist=["jobs"]), "jobs")
...
def test_setup_jobs_are_never_listed_twice(isolated_settings: Path) -> None:
    available_jobs = getattr(
        __import__("deeptutor.capabilities.setup.jobs", fromlist=["available_jobs"]),
        "available_jobs",
    )
