...
async def test_status_on_unknown_path_creates_nothing(tmp_path, monkeypatch):
    ...
    """
    MasteryStatusTool = getattr(
        __import__("deeptutor.capabilities.mastery.tools", fromlist=["MasteryStatusTool"]),
        "MasteryStatusTool",
    )
...
async def test_recording_tools_refuse_an_unbuilt_path_without_creating_it(tmp_path, monkeypatch):
    ...
    MasteryAssessTool = getattr(
        __import__("deeptutor.capabilities.mastery.tools", fromlist=["MasteryAssessTool"]),
        "MasteryAssessTool",
    )
    MasteryGradeTool = getattr(
        __import__("deeptutor.capabilities.mastery.tools", fromlist=["MasteryGradeTool"]),
        "MasteryGradeTool",
    )
    MasteryQuizTool = getattr(
        __import__("deeptutor.capabilities.mastery.tools", fromlist=["MasteryQuizTool"]),
        "MasteryQuizTool",
    )
