...
def material(reading_home: Path):
    ReadingStore = getattr(
        __import__("deeptutor.reading", fromlist=["ReadingStore"]), "ReadingStore"
    )
...
def _mode_context(**metadata) -> UnifiedContext:
    MODE_KEY = getattr(
        __import__("deeptutor.capabilities.reading.capability", fromlist=["MODE_KEY"]), "MODE_KEY"
    )
...
def test_nothing_open_means_no_viewport_seed_and_no_locate_pass() -> None:
    VIEWPORT_KEY = getattr(
        __import__("deeptutor.capabilities.reading.capability", fromlist=["VIEWPORT_KEY"]),
        "VIEWPORT_KEY",
    )
