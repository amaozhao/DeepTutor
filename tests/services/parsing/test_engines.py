...
def test_liteparse_signature_tracks_knobs() -> None:
    ...
    LiteParseConfig = __import__(
        "deeptutor.services.parsing.engines.liteparse.config", fromlist=["LiteParseConfig"]
    ).LiteParseConfig
...
def test_liteparse_config_rejects_unknown_image_mode_and_coerces_strings() -> None:
    RuntimeSettingsService = __import__(
        "deeptutor.services.config.runtime_settings", fromlist=["RuntimeSettingsService"]
...
def test_liteparse_leaves_foreign_image_links_alone(tmp_path, monkeypatch) -> None:
    ...
    LiteParseParser = __import__(
        "deeptutor.services.parsing.engines.liteparse.engine", fromlist=["LiteParseParser"]
    ).LiteParseParser
