...
from deeptutor.services.rag.pipelines.graphrag.provider import resolve_completion_provider


def _optional_graphrag():
    report_type = pytest.importorskip(
    ...
    adapter = __import__(
        ...
        fromlist=["completion_adapter"],
    )
    return report_type, adapter
...
def _load_completion(
    ...
    *,
    ...
    create_completion = pytest.importorskip("graphrag_llm.completion").create_completion
...
def test_real_graphrag_completion_falls_back_when_schema_type_is_unavailable(
    ...
    CommunityReportResponse, completion_adapter = _optional_graphrag()
...
def test_custom_anthropic_endpoint_uses_prompt_only_structured_output(
    ...
    litellm = pytest.importorskip("litellm")
    CommunityReportResponse, _completion_adapter = _optional_graphrag()
