...
def _isolated_data_root(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(multi_user_paths, "ADMIN_WORKSPACE_ROOT", root)
    monkeypatch.setattr(multi_user_paths, "USERS_ROOT", root / "users")
    monkeypatch.setattr(multi_user_paths, "SYSTEM_ROOT", root / "system")
...
async def test_reload_reconnects_a_server_whose_secret_changed(
    ...
    ToolRegistry = __import__(
        "deeptutor.runtime.registry.tool_registry", fromlist=["ToolRegistry"]
    ).ToolRegistry
    save_user_server = __import__(
        "deeptutor.services.mcp.user_config", fromlist=["save_user_server"]
    ).save_user_server
