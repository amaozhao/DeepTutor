...
from deeptutor.learning import pending as pending_ops
...
class LearningService:
        ...
        def register(tx):
            ...
            tx.emit(
                "interaction.registered",
                {
                    ...
                    "question": pending_ops.public_pending_question(pending).to_dict(),
                },
                session_id=session_id,
                turn_id=turn_id,
            )
        ...
        def grade(tx):
            ...
            if interaction.status == InteractionStatus.ANSWERED:
                if pending.question_type == "choice":
                    option_map = pending_ops.parse_options(pending.options)
                    if pending_ops.has_option_bodies(option_map):
                        graded_answer = (
                            pending_ops.resolve_choice_submission(raw_answer, option_map)
                            or raw_answer
                        )
                    else:
            ...
            if pending.question_type == "choice" and resolved_choice_options:
                pending.options = pending_ops.format_options(resolved_choice_options)
    ...
    def record_qualitative_for_path(
        ...
        *,
        ...
        def record(tx):
            kp, _, _ = policy.find_knowledge_point(tx.progress, kp_id)
            if kp is None:
                raise MasteryInteractionError(
                    f"Unknown objective {kp_id!r}; refresh mastery_status"
                )
            if kp.type not in policy.QUALITATIVE_TYPES:
                raise MasteryInteractionError(
