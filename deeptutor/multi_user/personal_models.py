...
def _codex_profile_is_current(profile: dict[str, Any]) -> bool:
    try:
        codex_auth = importlib.import_module("deeptutor.services.codex_auth")
        return codex_auth.get_codex_oauth_service().profile_matches_current_account(profile)
    except Exception:
        return False
...
def _personal_catalog_profiles() -> list[dict[str, Any]]:
    ...
    """
    context = importlib.import_module("deeptutor.multi_user.context")
    model_access = importlib.import_module("deeptutor.multi_user.model_access")
    user = context.get_current_user_or_none()
    ...
    return [
        ...
        and model_access.is_owner_bound(profile)
