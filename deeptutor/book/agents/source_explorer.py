...
from deeptutor.services.rag import provider_binding
from deeptutor.services.rag.factory import PAGEINDEX_OSS_PROVIDER, PAGEINDEX_PROVIDER
from deeptutor.services.rag.pipelines.pageindex import reasoning as pageindex_reasoning
...
class SourceExplorer(BaseAgent):
    ...
    def partition_knowledge_bases(kb_list: list[str]) -> tuple[list[str], list[str]]:
        ...
        for kb in kb_list:
            try:
                meta = knowledge_access.resolve_kb_metadata(kb)
    ...
    async def _retrieve_kb_chunks(
        ...
        *,
        ...
        for kb in kb_list:
            try:
                resource = knowledge_access.resolve_kb(kb, require_write=False)
                provider = provider_binding.resolve_bound_provider(
                    str(resource.base_dir), resource.name
                )
            except Exception:
                provider = ""
            (
        ...
        async def _one_query(kb: str, query: str) -> list[SourceChunk]:
            try:
                result = await rag_tool.rag_search(query=query, kb_name=kb)
            except Exception as exc:
    ...
    async def _retrieve_pageindex_chunks(
        ...
        *,
        ...
        async def read_one(kb: str) -> SourceChunk | None:
            try:
                result = await pageindex_reasoning.read_pageindex_with_agent(
                    kb_name=kb,
                    system_prompt=(
