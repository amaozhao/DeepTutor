...
async def optional_rag_lookup(*, query: str, ctx) -> RagLookup:
    ...
    try:
        resource = knowledge_access.resolve_kb(ctx.primary_kb, require_write=False)
        provider = provider_binding.resolve_bound_provider(str(resource.base_dir), resource.name)
    ...
    try:
        result = await rag_tool.rag_search(query=query, kb_name=ctx.primary_kb)
