...
def _base_dir() -> Path:
    ...
    return ensure_dir(get_path_service().get_user_root() / "workspace" / "tutorbot")
...
def get_shared_memory_dir() -> Path:
    """Public memory shared by DeepTutor and all bots for the active user."""
    from deeptutor.services.path_service import get_path_service

    return ensure_dir(get_path_service().get_memory_dir())
