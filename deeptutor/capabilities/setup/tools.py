...
class InspectSetupTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        access = importlib.import_module("deeptutor.capabilities.setup.access")
        binding = importlib.import_module("deeptutor.capabilities.setup.binding")
        jobs_module = importlib.import_module("deeptutor.capabilities.setup.jobs")
        settings_spec = importlib.import_module("deeptutor.services.config.settings_spec")

        area = str(kwargs.get("area") or "").strip().lower()
        rows: list[dict[str, Any]] = []
        for spec in settings_spec.specs_for_area(area):
            decision = access.can_write(spec.scope)
            try:
                current = spec.read()
        ...
        jobs_decision = access.can_write("global")
        jobs = [
            {
                **job,
            ...
            for job in jobs_module.available_jobs()
        ]

        payload = {
            "settings": rows,
            "gaps": [gap.to_dict() for gap in binding.setup_gaps()],
            "jobs_available": jobs,
        }
...
class ApplySettingTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        apply_module = importlib.import_module("deeptutor.capabilities.setup.apply")
        ...
        outcome = await apply_module.apply_setting(key, value)
...
class RunSetupJobTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        decision = access.can_write("global")
        ...
        outcome = await jobs_module.run_job(action, engine, on_line=_on_line)
        payload = outcome.to_dict()
        if not outcome.ok:
            return ToolResult(
                content=json.dumps(payload, ensure_ascii=False),
