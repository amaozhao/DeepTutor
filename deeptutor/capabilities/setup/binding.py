...
def setup_gaps() -> tuple[SetupGap, ...]:
    ...
    gaps: list[SetupGap] = []
    try:
        specs = settings_spec.setting_specs()
...
def _intro_pending(context: UnifiedContext) -> bool:
    ...
    if context.conversation_history:
        return False
    try:
        interface = importlib.import_module("deeptutor.services.settings.interface_settings")
        if bool(interface.get_ui_settings().get(INTRO_SHOWN_KEY)):
            return False
...
def mark_intro_shown() -> None:
    ...
    try:
        interface = importlib.import_module("deeptutor.services.settings.interface_settings")
        interface.set_ui_setting(INTRO_SHOWN_KEY, True)
