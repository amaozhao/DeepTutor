...
def available_jobs() -> list[dict[str, Any]]:
    ...
    downloadable = install.model_downloadable_engines()
    for engine in factory.list_engines():
        engine_id = str(engine.get("id") or "")
        if not engine_id:
            continue
        installed = factory.is_engine_available(engine_id)
    ...
    if "mineru" not in downloadable and factory.is_engine_available("mineru"):
        out.append(
            {
                "action": "download_models",
                "engine": "mineru",
...
def _start_install(engine: str) -> tuple[bool, str, Any]:
    ...
    specs = install.ENGINE_PIP_SPECS.get(engine)
    if not specs:
        return False, f"There is no one-step install for the '{engine}' engine.", None
    manager = install.get_background_job_manager()
...
def _start_model_download(engine: str) -> tuple[bool, str, Any]:
    ...
    if engine not in install.model_downloadable_engines():
        return False, f"The '{engine}' engine has no model download step.", None
    cmd = install.resolve_model_downloader(engine)
    if not cmd:
        return (
            False,
            (
...
def _start_mineru_download() -> tuple[bool, str, Any]:
    settings_service = importlib.import_module("deeptutor.services.config.runtime_settings")
    models = importlib.import_module("deeptutor.services.parsing.engines.mineru.models")

    settings = settings_service.get_runtime_settings_service().load_document_parsing()
    engine_settings = (settings.get("engines") or {}).get("mineru") or {}
    resolved = models.resolve_models_downloader(str(engine_settings.get("local_cli_path") or ""))
    ...
    manager = models.get_model_download_manager()
