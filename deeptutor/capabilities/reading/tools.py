...
class _ReadingToolBase(BaseTool):
    ...
    def _store():
        ...
        return importlib.import_module("deeptutor.reading").ReadingStore()
...
def _guard(func):
    ...
    async def wrapper(self: _ReadingToolBase, **kwargs: Any) -> ToolResult:
        reading = importlib.import_module("deeptutor.reading")
...
class MaterialOutlineTool(_ReadingToolBase):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        store = self._store()
        text = await asyncio.to_thread(reading.render_outline, store, material_id)
...
class SearchMaterialTool(_ReadingToolBase):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        result = await asyncio.to_thread(
            reading.search_material, store, material_id, query, limit=limit
        )
...
class ReadMaterialTool(_ReadingToolBase):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        store = self._store()
        manifest = await asyncio.to_thread(store.manifest, material_id)
        rendered = await asyncio.to_thread(reading.render_units, store, material_id, spec)
...
class ReaderGotoTool(_ReadingToolBase):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        check = await asyncio.to_thread(reading.verify_quote, store, material_id, locator, quote)
...
class ReaderAnnotateTool(_ReadingToolBase):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        annotation = reading.Annotation(
            ...
            locator=target,
            kind="highlight",
            color=colour if colour in reading.ANNOTATION_COLORS else "yellow",
            quote=quote,
