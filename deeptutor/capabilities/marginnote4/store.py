...
def default_db_path(kb_name: str, *, path_service: Any = None) -> Path:
    ...
    if path_service is None:
        path_service = importlib.import_module("deeptutor.services.path_service").get_path_service()
...
def _kb_metadata(kb_name: str) -> dict[str, Any] | None:
    try:
        return importlib.import_module("deeptutor.multi_user.knowledge_access").resolve_kb_metadata(
            kb_name
        )
