...
from deeptutor.capabilities.mastery import binding as mastery_binding
    ...
    *,
    ...
    try:
        await asyncio.wait_for(
            session_services.get_sqlite_session_store().upsert_notebook_entries(session_id, [item]),
            timeout=5.0,
        )
    except Exception:
        logger.warning(
...
class MasterySwitchTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        try:
            active = await mastery_binding.rebind_active_path(
                path_id=requested,
                session_id=_resolve_session_id(kwargs),
                turn_id=_resolve_turn_id(kwargs),
                bind_turn=kwargs.get("_bind_active_path"),
            )
        except mastery_binding.PathBindingError as exc:
            return ToolResult(content=str(exc), success=False)
        return _json_result(
            {
                "status": "switched",
...
class MasteryLeaveTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        previous = _resolve_path_id(kwargs)
        try:
            active = await mastery_binding.leave_active_path(
                session_id=_resolve_session_id(kwargs),
                turn_id=_resolve_turn_id(kwargs),
                bind_turn=kwargs.get("_bind_active_path"),
            )
