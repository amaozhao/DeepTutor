...
def _get_learning_service_class():
    return importlib.import_module("deeptutor.learning.service").LearningService
...
def _bind_pending_ask_user_args(kwargs: dict[str, Any], path_id: str) -> dict[str, Any]:
    ...
    if not path_id:
        return kwargs
    try:
        pending_module = importlib.import_module("deeptutor.learning.pending")
        storage_module = importlib.import_module("deeptutor.learning.storage")
        progress = storage_module.LearningStore().load(path_id)
    ...
    updated = dict(kwargs)
    updated["questions"] = [pending_module.public_pending_question(pending).to_ask_user_dict()]
...
class MasteryLoopCapability:
    ...
    async def on_user_pause(
        ...
        await asyncio.to_thread(
            _get_learning_service_class()().mark_question_awaiting,
            path_id,
    ...
    async def on_user_resume(
        ...
        *,
        ...
        if not path_id:
            return
        await asyncio.to_thread(
            _get_learning_service_class()().record_question_answer,
            path_id,
