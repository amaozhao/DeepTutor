...
def _note_sort(name: Any) -> int:
    ...
    notes = importlib.import_module("deeptutor.services.rag.pipelines.ima.notes")

    return {
        "updated": notes.SORT_BY_UPDATED,
        "created": notes.SORT_BY_CREATED,
        "title": notes.SORT_BY_TITLE,
    }.get(str(name or "updated"), notes.SORT_BY_UPDATED)
...
class _ImaTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        config = importlib.import_module("deeptutor.services.rag.pipelines.ima.config")
        envelope = importlib.import_module("deeptutor.services.rag.pipelines.ima.envelope")
        ...
        except config.ImaNotConfiguredError as exc:
            return _err(str(exc))
        except Exception:
            return _err(
...
class ImaReadTool(_ImaTool):
    ...
    async def _run(self, client, binding, kwargs):
        media_ops = importlib.import_module("deeptutor.services.rag.pipelines.ima.media")
        source_policy = importlib.import_module("deeptutor.services.rag.pipelines.ima.sources")
