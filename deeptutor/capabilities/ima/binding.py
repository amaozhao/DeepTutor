...
def resolve_client(kb_ref: str, *, for_write: bool = False):
    ...
    client_module = importlib.import_module("deeptutor.services.rag.pipelines.ima.client")
    config_module = importlib.import_module("deeptutor.services.rag.pipelines.ima.config")
    provider_binding = importlib.import_module("deeptutor.services.rag.provider_binding")

    resource = access.resolve_kb(str(kb_ref), require_write=for_write)
    entry = provider_binding.load_kb_config_entry(resource.base_dir, resource.name)
    return client_module.ImaClient(config_module.resolve_kb_config(entry))
...
def _resolve(context: UnifiedContext) -> tuple[ImaBinding, ...]:
    ...
    for raw in context.knowledge_bases or []:
        ...
        if not ref or ref in seen:
            continue
        seen.add(ref)
        meta = access.resolve_kb_metadata(ref)
