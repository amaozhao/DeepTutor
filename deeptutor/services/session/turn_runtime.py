...
class _TurnExecution:
    ...
    context_bootstrap_progress_emitted: bool = False
...
class TurnRuntimeManager:
    ...
    async def start_turn(self, payload: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        if (
            ...
            execution.context_bootstrap_progress_emitted = True
        async with self._lock:
    ...
    async def _run_turn(self, execution: _TurnExecution) -> None:
        ...
        try:
            ...
            async def _emit_context_event(event: StreamEvent) -> None:
                if (
                    execution.context_bootstrap_progress_emitted
                    and event.type == StreamEventType.PROGRESS
                    and event.source == "context"
                    and event.stage == "summarizing"
                    and event.content == "summarize context"
                ):
                    return
            ...
            if is_chat_capability:
                ...
                resolved_notebook_records = (
                    get_records_by_references_for_current_user(notebook_references)
                    if notebook_references
                    else []
                )
                ...
                if notebook_references:
                    referenced_records = get_records_by_references_for_current_user(
                        notebook_references
                    )
