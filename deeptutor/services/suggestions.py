...
def _cache_path():
    path_service = importlib.import_module("deeptutor.services.path_service")
    directory = path_service.get_path_service().get_workspace_dir() / "suggestions"
...
def _output_language() -> str:
    ...
    try:
        ...
        return settings.get_response_language(default="en")
    except Exception:
...
def _trace_count() -> int:
    ...
    settings = importlib.import_module("deeptutor.services.settings.starter_settings")

    try:
        return int(settings.get_starter_settings()["trace_count"])
    except Exception:
        logger.debug("suggestions: starter settings unreadable", exc_info=True)
        return settings.DEFAULT_TRACE_COUNT
...
def _render_l3(cap: int) -> str:
    ...
    """
    memory = importlib.import_module("deeptutor.services.memory")
...
def _collect_material(trace_count: int) -> _Material:
    ...
    nothing about that.
    """
    recall = importlib.import_module("deeptutor.services.memory.recall")
...
def _sanitize(raw: str, language: str = "en") -> tuple[Suggestion, ...]:
    ...
    """
    json_parser = importlib.import_module("deeptutor.utils.json_parser")
    decoded = json_parser.parse_json_response(raw, fallback=None)
