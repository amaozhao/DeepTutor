...
class CodeBuddyAuthService:
    ...
    async def logout(self) -> dict[str, Any]:
        ...
        credentials = importlib.import_module("deeptutor.services.codebuddy_credentials")
        agentic_client.reset_agentic_client_pool()
        async with self._lock:
            if credentials.load_credentials() is not None:
    ...
    async def _probe_local_login(self) -> bool:
        ...
        """
        credentials_module = importlib.import_module("deeptutor.services.codebuddy_credentials")
        credentials = credentials_module.load_credentials()
        if credentials is None:
            return False
        try:
            label = await credentials_module.probe_account(credentials)
...
async def _start_sdk_authenticate() -> Any:
    try:
        sdk = importlib.import_module("codebuddy_agent_sdk")
    except ImportError as exc:
        raise ImportError("codebuddy-agent-sdk is not installed") from exc
    return await sdk.authenticate(timeout=300.0)
