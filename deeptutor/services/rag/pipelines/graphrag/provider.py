...
def resolve_persisted_completion_provider(model_config: Any) -> str:
    ...
    try:
        config_service = importlib.import_module("deeptutor.services.config")
        service = config_service.get_model_catalog_service()
        ...
        if len(matches) == 1:
            ...
            resolved = config_service.resolve_llm_runtime_config(
                catalog=catalog,
                service=service,
