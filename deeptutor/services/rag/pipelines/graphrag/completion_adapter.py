...
def _format_response(response: Any, response_format: type[BaseModel]) -> Any:
    try:
        response.formatted_response = importlib.import_module(
            "graphrag_llm.utils"
...
def _get_adapter_class() -> type:
    ...
    with _CAPABILITY_LOCK:
        ...
        LiteLLMCompletion = importlib.import_module(
            "graphrag_llm.completion.lite_llm_completion"
        ).LiteLLMCompletion
...
def register_completion_adapter() -> None:
    ...
    importlib.import_module("graphrag_llm.completion").register_completion(
        COMPLETION_TYPE, initializer, scope="singleton"
    )
