...
def _load_config(root_dir: Path):
    importlib.import_module(
        "deeptutor.services.rag.pipelines.graphrag.completion_adapter"
    ).register_completion_adapter()
    config = importlib.import_module("graphrag.config.load_config").load_config(
...
def _create_probe_completion(llm_cfg: Any):
    ...
    create_completion = importlib.import_module("graphrag_llm.completion").create_completion
    ModelConfig = importlib.import_module("graphrag_llm.config").ModelConfig
    register_completion_adapter = importlib.import_module(
        "deeptutor.services.rag.pipelines.graphrag.completion_adapter"
    ).register_completion_adapter
    ...
    return create_completion(model_config), response_module.CommunityReportResponse
...
def _create_configured_probe_completion(config: Any):
    ...
    return (
        ...
        response_module.CommunityReportResponse,
    )
...
def _create_probe_embedding(config: Any) -> tuple[Any, int]:
    ...
    create_embedding = importlib.import_module("graphrag_llm.embedding").create_embedding
