...
def _read_ui(key: str, default: str) -> Callable[[], str]:
    def _read() -> str:
        settings = importlib.import_module("deeptutor.services.settings.interface_settings")
        value = settings.get_ui_settings().get(key, default)
...
def _write_ui(key: str) -> Callable[[str], None]:
    def _write(value: str) -> None:
        settings = importlib.import_module("deeptutor.services.settings.interface_settings")
        settings.set_ui_setting(key, value)
...
def _read_parsing_engine() -> str:
    runtime_settings = importlib.import_module("deeptutor.services.config.runtime_settings")
    settings = runtime_settings.get_runtime_settings_service().load_document_parsing()
    return str(settings.get("engine") or "text_only")


def _write_parsing_engine(value: str) -> None:
    runtime_settings = importlib.import_module("deeptutor.services.config.runtime_settings")
    service = runtime_settings.get_runtime_settings_service()
...
def _parsing_engine_choices() -> tuple[SettingChoice, ...]:
    ...
    """
    install = importlib.import_module("deeptutor.services.parsing.engines._install")
    factory = importlib.import_module("deeptutor.services.parsing.engines.factory")

    active = _read_parsing_engine()
    installable = install.installable_engines()
    out: list[SettingChoice] = []
    for engine in factory.list_engines():
...
def _validate_parsing_engine(value: str) -> str | None:
    factory = importlib.import_module("deeptutor.services.parsing.engines.factory")
    if factory.is_engine_available(value):
        return None
    return (
...
def _catalog_service():
    catalog = importlib.import_module("deeptutor.services.config.model_catalog")
    return catalog.get_model_catalog_service()
...
def _catalog_profile_choices(service_name: str) -> Callable[[], tuple[SettingChoice, ...]]:
    def _choices() -> tuple[SettingChoice, ...]:
        provider_runtime = importlib.import_module("deeptutor.services.config.provider_runtime")
        ...
        for profile in catalog.get("services", {}).get(service_name, {}).get("profiles", []) or []:
            ...
            spec = provider_runtime.SEARCH_PROVIDERS.get(provider)
...
def _clear_runtime_caches() -> None:
    ...
    """
    try:
        llm = importlib.import_module("deeptutor.services.llm")
    ...
    try:
        ...
        await asyncio.wait_for(
            llm.complete(
                ...
                **llm.get_token_limit_kwargs(resolved.model, max_tokens=64),  # type: ignore[arg-type]
            ),
...
async def _probe_embedding(value: str) -> ProbeResult:
    ...
    provider_runtime = importlib.import_module("deeptutor.services.config.provider_runtime")
    embedding_client = importlib.import_module("deeptutor.services.embedding.client")
    embedding_config = importlib.import_module("deeptutor.services.embedding.config")

    started = time.monotonic()
    try:
        resolved = provider_runtime.resolve_embedding_runtime_config(
