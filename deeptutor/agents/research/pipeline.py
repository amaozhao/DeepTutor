...
from deeptutor.agents.chat.agentic_pipeline import _format_user_reply_body, _normalise_user_reply
...
class ResearchPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        if self.kb_name:
            try:
                kb_types = importlib.import_module("deeptutor.knowledge.kb_types")
                knowledge_access = importlib.import_module("deeptutor.multi_user.knowledge_access")
                meta = knowledge_access.resolve_kb_metadata(self.kb_name)
                if meta and meta.get("type") == kb_types.OBSIDIAN_KB_TYPE:
    ...
    async def _prepare_pageindex_tools(self) -> None:
        self._pageindex_tool_context = (
            await _get_pageindex_tools_module().build_pageindex_tool_context(
                self.kb_name,
