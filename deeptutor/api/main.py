...
def _install_routers(api: FastAPI) -> None:
    ...
    api.include_router(outputs.router, prefix="/api/outputs", tags=["outputs"])
    ...
    api.include_router(
        reading.router, prefix="/api/v1/reading", tags=["reading"], dependencies=auth_deps
    )
    api.include_router(
    ...
    api.include_router(settings.public_router, prefix="/api/v1/settings", tags=["settings"])
    api.include_router(
    ...
    # Pairing/management routes carry auth in-router; sync and heartbeat use
    # device-token auth because the MarginNote add-on has no browser session.
    api.include_router(
        marginnote4.router,
