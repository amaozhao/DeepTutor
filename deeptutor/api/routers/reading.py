...
def _get_reading_extract_module():
    return importlib.import_module("deeptutor.reading.extract")


def _get_document_extractor_module():
    return importlib.import_module("deeptutor.utils.document_extractor")
...
async def supported_formats() -> SupportedFormats:
    ...
    return SupportedFormats(
        extensions=sorted(_get_document_extractor_module().SUPPORTED_DOC_EXTENSIONS),
        max_bytes=MAX_MATERIAL_BYTES,
        raw_view_extensions=sorted(_get_reading_extract_module().RAW_VIEW_EXTENSIONS),
    )
