...
def _llm_options_with_source(payload: dict[str, Any], source: str) -> dict[str, Any]:
...
def _require_settings_admin(user: Any | None = None) -> None:
    if user is not None:
        if _is_admin_user(user):
            return
        raise HTTPException(
...
async def get_llm_options(user: Any | None = None):
    if user is not None:
        source = "admin" if _is_admin_user(user) else "user"
        return _llm_options_with_source(list_llm_options(_catalog_for_user(user)), source)
    if not get_current_user().is_admin:
        return allowed_llm_options()
    return _llm_options_with_source(list_llm_options(get_model_catalog_service().load()), "admin")
...
async def cancel_service_test(service: str, run_id: str, user: Any | None = None):
    _require_settings_admin(user)
