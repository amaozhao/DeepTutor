...
from deeptutor.multi_user import shared_state as shared_state_services
...
def _container_health() -> tuple[int, dict[str, object]]:
    ...
    if external_state_configured:
        shared_state_store = shared_state_services.health_status()
    failures: list[str] = []
...
async def get_system_status():
    ...
    if result["deployment"]["shared_state"]["external_state_configured"]:
        result["shared_state_store"] = shared_state_services.health_status()
    ...
    if not get_current_user().is_admin:
        ...
        result["auth_store"].pop("path", None)
