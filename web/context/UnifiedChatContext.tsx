...
export function UnifiedChatProvider({ children }: { children: React.ReactNode }) {
    ...
    return messages
      ...
      .map(message => {
        ...
        return {
          id: message.id,
          role: message.role,
          content: message.role === 'assistant' ? normalizeMarkdownForDisplay(raw) : raw,
          capability: message.capability || '',
          events: Array.isArray(message.events) ? message.events : [],
          attachments,
          parentMessageId:
            message.parent_message_id === undefined ? null : message.parent_message_id,
        ...
        client: new UnifiedWSClient(
          ...
          () => {
            ...
            if (session?.isStreaming) {
              ...
              notify(i18n.t('Connection lost while generating. Please retry your message.'), {
                tone: 'error',
                durationMs: 6000,
              })
            }
  ...
  const loadSession = useCallback(
    async (
      ...
      dispatch({
        ...
        personaSelection:
          typeof session.preferences?.persona === 'string' ? session.preferences.persona : '',
  ...
  const sendMessage = useCallback(
    (
      ...
      const effectiveLanguage = replaySnapshot?.language ?? readStoredLanguage()
      ...
      const effectivePersona = replaySnapshot?.persona ?? persona ?? session.personaSelection ?? ''
      const effectiveMemoryReferences = replaySnapshot?.memoryReferences ?? memoryReferences
      const effectiveBookReferences = replaySnapshot?.bookReferences ?? options?.bookReferences
      ...
      const effectiveNotebookReferences = replaySnapshot?.notebookReferences ?? notebookReferences
      const effectiveHistoryReferences = replaySnapshot?.historyReferences ?? historyReferences
      ...
      const requestSnapshot: MessageRequestSnapshot = replaySnapshot ?? {
        ...
        ...(effectiveQuestionNotebookReferences?.length
          ...
          : {}),
        ...(effectiveBookReferences?.length ? { bookReferences: effectiveBookReferences } : {}),
      ...
      const visible = buildVisiblePath(session.messages, session.selectedBranches).messages
      const tipId = tipMessageId(visible)
      const localParentId = options?.parentMessageId !== undefined ? options.parentMessageId : tipId
      ...
      sendThroughRunner(key, {
        ...
        ...(effectiveQuestionNotebookReferences?.length
          ...
          : {}),
        ...(effectiveBookReferences?.length ? { book_references: effectiveBookReferences } : {}),
  ...
  const submitUserReply = useCallback(
    (
      ...
      const session = currentState.sessions[key]
      const turnId = session?.activeTurnId
      const pendingAskUser = session ? hasPendingAskUserInMessages(session.messages, turnId) : false
  ...
  const switchBranch = useCallback((parentMessageId: number | null, childId: number) => {
    const currentState = stateRef.current
    const key = currentState.selectedKey
    if (!key) return
    const session = currentState.sessions[key]
