...
export function createSessionEntry(key: string, sessionId: string | null = null): SessionEntry {
  return {
    key,
    sessionId,
    sessionTitle: '',
...
export function chatReducer(state: ProviderState, action: Action): ProviderState {
  switch (action.type) {
    ...
    case 'STREAM_EVENT': {
      ...
      if ((last?.events || []).some(event => isSameTurnEvent(event, action.event))) {
        return state
      }
    ...
    case 'SELECT_SESSION':
      if (!state.sessions[action.key] || state.selectedKey === action.key) return state
      return { ...state, selectedKey: action.key }
    case 'LOAD_SESSION':
    case 'REVALIDATE_SESSION': {
      if (action.type === 'REVALIDATE_SESSION') {
        const local = state.sessions[action.key]
        if (!local || local.isStreaming || local.status === 'running') return state
      }
      const existing =
        state.sessions[action.key] ?? createSessionEntry(action.key, action.sessionId)
      return {
        ...state,
        selectedKey: action.type === 'LOAD_SESSION' ? action.key : state.selectedKey,
        sessions: {
          ...state.sessions,
          [action.key]: {
            ...existing,
            key: action.key,
            sessionId: action.sessionId,
            sessionTitle: action.title !== undefined ? action.title : existing.sessionTitle,
