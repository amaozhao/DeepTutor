...
type RunnerClient = Pick<
  UnifiedWSClient,
  'connected' | 'connect' | 'disconnect' | 'send' | 'setResumeState'
>
...
export function moveRunner(runners: ChatRunnerMap, oldKey: string, newKey: string): void {
...
export function effectiveRunnerKey(runners: ChatRunnerMap, runnerKey: string): string {
...
export function runnerConnectionFailedAction(key: string): Extract<Action, { type: 'STREAM_END' }> {
  return { type: 'STREAM_END', key, status: 'failed' }
}

export function runnerClosedFailedAction(key: string): Extract<Action, { type: 'STREAM_END' }> {
  return { type: 'STREAM_END', key, status: 'failed' }
}
...
export function sessionMetaAction(
  ...
  return title ? { type: 'SET_SESSION_TITLE', key, title } : { type: 'BUMP_SIDEBAR_REFRESH' }
}
