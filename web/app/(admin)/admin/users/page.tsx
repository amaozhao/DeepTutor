...
import { downloadUserExport, fetchAdminAuditEvents } from '@/features/multi-user/api'
import type { AuditEvent, DeleteDataAction } from '@/features/multi-user/types'
...
export default function AdminUsersPage() {
  ...
  const [confirmTarget, setConfirmTarget] = useState<{
    kind: 'delete' | 'promote' | 'demote' | 'disable' | 'enable' | 'revoke'
    user: UserRecord
  } | null>(null)
  ...
  const [deleteDataAction, setDeleteDataAction] = useState<DeleteDataAction>('keep')
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const [userData, inviteData] = await Promise.all([listUsers(), listInvites()])
      setUsers(userData)
  ...
  async function handleCreateSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      setCreateError(e instanceof Error ? e.message : t('Failed to create user'))
    } finally {
  ...
  async function copyInvite(invite: InviteRecord) {
    try {
      ...
      setActionError(
        tr('邀请码已创建，但无法复制链接。', 'Invite created, but the link could not be copied.')
      )
    }
  }
  ...
  async function handleUsersCsvExport() {
    ...
    try {
      ...
      setActionError(
        e instanceof Error ? e.message : tr('导出用户列表失败', 'Failed to export users')
      )
    } finally {
    ...
    >
        ...
        <div className="mb-8">
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
              ...
              >
                <Download size={14} />
                {exportingUsersCsv ? tr('导出中…', 'Exporting…') : tr('导出 CSV', 'Export CSV')}
              </button>
              <button
              ...
              >
                <Upload size={14} />
                {importingUsersCsv ? tr('导入中…', 'Importing…') : tr('导入 CSV', 'Import CSV')}
              </button>
              <input
                ref={importInputRef}
                type="file"
        ...
        {!loading && !error && (
          <section className="mb-4 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4 shadow-sm">
            ...
            >
              ...
              >
                <KeyRound size={14} />
                {inviteSubmitting ? tr('创建中…', 'Creating…') : tr('创建邀请', 'Create invite')}
              </button>
            </form>
                  ...
                  return (
                    ...
                    >
                      <div className="min-w-0">
                        <p className="truncate font-mono text-[var(--foreground)]">{invite.code}</p>
                          ...
                          <div className="flex items-center justify-end gap-1.5">
                            ...
                            {canManageAssignments && (
                              <button
                                ...
                                title={tr(`导出 ${user.username}`, `Export ${user.username}`)}
                                className="rounded-lg p-1.5 text-[var(--muted-foreground)]
                                         hover:bg-[var(--background)] hover:text-[var(--foreground)]
                            ...
                            <button
                              ...
                              title={
                                isSelf
                                  ...
                                  : tr(`让 ${user.username} 退出登录`, `Sign out ${user.username}`)
                              }
                              className="rounded-lg p-1.5 text-[var(--muted-foreground)]
                                       hover:bg-[var(--background)] hover:text-[var(--foreground)]
                                       disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                            ...
                            <button
                              ...
                              title={
                                isSelf
                                  ? tr('不能禁用自己的账号', 'Cannot disable your own account')
                                  : user.disabled
                                    ? tr(`启用 ${user.username}`, `Enable ${user.username}`)
                                    : tr(`禁用 ${user.username}`, `Disable ${user.username}`)
                              }
                              className="rounded-lg p-1.5 text-[var(--muted-foreground)]
                                       hover:bg-[var(--background)] hover:text-[var(--foreground)]
                                       disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
            ...
            <div className="flex items-center gap-3 rounded-xl border border-[var(--border)] bg-[var(--background)]/50 px-3 py-2.5">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-[var(--muted-foreground)]">
                  {t('{{role}} · joined {{date}}', {
                    role: confirmTarget.user.role === 'admin' ? t('Admin') : t('User'),
            ...
            {confirmTarget.kind === 'delete' && (
              <label className="mt-3 block text-xs text-[var(--muted-foreground)]">
                ...
                <select
                  ...
                  onChange={e => setDeleteDataAction(e.target.value as DeleteDataAction)}
      ...
      {resetTarget && (
        ...
        >
          ...
          >
            ...
            <p className="mb-3 text-sm text-[var(--muted-foreground)]">{resetTarget.username}</p>
            ...
            {resetError && <p className="mb-3 text-xs text-red-500">{resetError}</p>}
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              >
                {resetSubmitting ? tr('重置中…', 'Resetting…') : tr('重置', 'Reset')}
              </button>
            </div>
          </form>
        </div>
                  ...
                  {auditEvents.map((event, index) => (
                    ...
                    >
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <span className="shrink-0 text-xs text-[var(--muted-foreground)]">
                          {event.time ? formatDate(event.time, lang) : '—'}
                        </span>
                      </div>
      ...
      {showCreateDialog && (
        ...
        >
          ...
          >
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-base font-semibold text-[var(--foreground)]">{t('Add user')}</h2>
              <button
                type="button"
                onClick={closeCreateDialog}
            ...
            {createError && <p className="mb-3 text-xs text-red-500">{createError}</p>}
