...
const CARD = path.resolve(process.cwd(), 'components/settings/CodeBuddyAuthCard.tsx')
...
test('CodeBuddy renders a login card without changing other OAuth cards', () => {
  ...
  assert.match(profile, /providerValue === ['"]codebuddy['"]/)
  assert.match(profile, /<CodeBuddyAuthCard/)
  assert.match(editor, /<CodexOAuthCard/)
  assert.match(profile, /isCodexOAuth \|\| isCodeBuddyAuth/)
  assert.match(profile, /!isCodexOAuth && !isCodeBuddyAuth/)
