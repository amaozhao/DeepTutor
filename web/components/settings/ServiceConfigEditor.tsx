...
export function ServiceConfigEditor({ service }: { service: ServiceName }) {
  ...
  const activeProviderValue =
    service === 'search' ? activeProfile?.provider || '' : activeProfile?.binding || ''
  ...
  const searchProviderRaw =
    service === 'search' ? (activeProfile?.provider || '').trim().toLowerCase() : ''
  const showSearchProviderWarning = service === 'search' && Boolean(searchProviderRaw)
  const searchProviderOption = (providers.search || []).find(
    option => option.value === searchProviderRaw
  )
  const isDeprecatedSearchProvider = searchProviderOption?.status === 'deprecated'
  const isSupportedSearchProvider = Boolean(searchProviderOption) && !isDeprecatedSearchProvider
  const supportedSearchProviderNames = (providers.search || [])
    .filter(option => option.status !== 'deprecated' && option.value !== 'none')
  ...
  const syncProviderModels = async (
    ...
    try {
      ...
      mutateCatalog(next => {
        ...
        const existing = new Map(profile.models.map(model => [model.model, model]))
        ...
        if (!profile.models.some(model => model.id === target.active_model_id)) {
  ...
  const commitModelRename = (modelId: string) => {
    const fallbackIndex = activeProfile?.models.findIndex(model => model.id === modelId) ?? -1
  ...
  const commitProfileRename = (profileId: string) => {
    ...
    if (nextName) {
      mutateCatalog(next => {
        const profile = next.services[service].profiles.find(item => item.id === profileId)
          ...
          <aside className="sticky top-4 self-start rounded-xl border border-[var(--border)]/60 bg-[var(--card)]/40 p-2">
            ...
            <div className="space-y-0.5">
              {draft.services[service].profiles.map(profile => {
                const isActive = profile.id === draft.services[service].active_profile_id
                const profileDetail = activeProfileDetail(profile, service, t)
                const isManagedProfile = isManagedCodexProfile(profile)
                const isEditing = editingProfileId === profile.id && !isManagedProfile
                return (
                  <div
                    key={profile.id}
                    role="button"
                    ...
                    title={isEditing || isManagedProfile ? undefined : t('Double-click to rename')}
                  ...
                  >
                    ...
                    {isEditing ? (
                      ...
                      <div className="flex items-center gap-1.5">
                        <ProviderIcon
                          provider={service === 'search' ? profile.provider : profile.binding}
                          size={13}
                        />
                        <div
            ...
            <div className="rounded-xl border border-[var(--border)] p-5">
              ...
              <ProfileFields
                ...
                onProviderChanged={(provider, previousProvider) => {
                  ...
                  if (provider.value === 'codebuddy' || previousProvider === 'codebuddy') {
                    const profileId = activeProfile.id
                    mutateCatalog(next => {
                      const target = next.services.llm
                      const profile = target.profiles.find(item => item.id === profileId)
                ...
                <div className="mb-4 flex items-center justify-between gap-2">
                  ...
                  {!isCodexOAuth && (
                    <div className="flex items-center gap-2">
                      {service === 'llm' && activeProfile.binding === 'codebuddy' && (
                        <button
                          type="button"
                        ...
                        >
                          <RefreshCw className={`h-3 w-3 ${modelsSyncing ? 'animate-spin' : ''}`} />
                          {t('Sync models')}
                        </button>
                      )}
                      <button
                ...
                {activeProfile.models.length > 0 && (
                  <div className="mb-4 flex flex-wrap items-center gap-1.5">
                    {activeProfile.models.map((model, index) => {
                      const isActive = model.id === draft.services[service].active_model_id
                      const label =
                        (model.name || '').trim() || defaultModelLabel(language, index + 1)
                      const metric =
                        service === 'llm'
                        ...
                        <div key={model.id} className="min-w-0">
                          {editingModelId === model.id && !isCodexOAuth ? (
                            ...
                            <button
                              ...
                              title={isCodexOAuth ? undefined : t('Double-click to rename')}
                    ...
                    {!isCodexOAuth && (
                      <div>
                        ...
                        <input
                          ...
                          onChange={e => updateModelField(service, 'model', e.target.value)}
                          placeholder="gpt-4o"
                        />
                      </div>
                    )}
                          ...
                          <>
                            <div>
                              ...
                              <input
                                ...
                                onChange={e => updateContextWindowField(e.target.value)}
                                placeholder="65536"
                              />
                    ...
                    {service === 'embedding' && (
                      <div>
                        <div className="mb-1.5 flex items-center justify-between gap-2">
                          ...
                          <label className="inline-flex cursor-pointer items-center gap-1.5 text-[11px] text-[var(--muted-foreground)] select-none">
                            <input
                              ...
                              onChange={e =>
                                updateModelBoolField(service, 'send_dimensions', e.target.checked)
                              }
                            />
                        ...
                        <DimensionField
                          ...
                          onChangeDimension={value => updateModelField(service, 'dimension', value)}
                        />
                      </div>
                    )}
                    {service === 'tts' && (
                      <>
                        <div>
                          ...
                          <input
                            ...
                            onChange={e => updateModelField(service, 'voice', e.target.value)}
                            placeholder="alloy"
                          />
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <select
                              ...
                              onChange={e =>
                                updateModelField(service, 'response_format', e.target.value)
                              }
                            >
                              {['mp3', 'wav', 'opus', 'aac', 'flac', 'pcm'].map(fmt => (
                                <option className={selectOptionClass} key={fmt} value={fmt}>
                                  {fmt}
                                </option>
                              ))}
                            </select>
                      ...
                      <>
                        <div>
                          ...
                          <input
                            ...
                            onChange={e => updateModelField(service, 'size', e.target.value)}
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <input
                              ...
                              onChange={e => updateModelField(service, 'quality', e.target.value)}
                              placeholder={t('quality (e.g. hd)')}
                            />
                            <input
                              className={inputClass}
                              value={activeModel.style || ''}
                              onChange={e => updateModelField(service, 'style', e.target.value)}
                      ...
                      <>
                        <div>
                          ...
                          <input
                            ...
                            onChange={e =>
                              updateModelField(service, 'aspect_ratio', e.target.value)
                            }
                            placeholder="16:9"
                          />
                          <p className="mt-1.5 text-[11px] text-[var(--muted-foreground)]">
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <input
                              ...
                              onChange={e => updateModelField(service, 'duration', e.target.value)}
                            ...
                            <input
                              ...
                              onChange={e =>
                                updateModelField(service, 'resolution', e.target.value)
                              }
                              placeholder="720p"
                            />
                          </div>
              ...
              <div className="flex items-center justify-between px-5 py-3.5">
                ...
                <div className="ml-3 flex items-center gap-3">
                  ...
                  <button
                    ...
                    aria-label={
                      diagnosticsOpen ? t('Collapse diagnostics') : t('Expand diagnostics')
                    }
                    aria-expanded={diagnosticsOpen}
                  >
                    <ChevronDown
              ...
              {diagnosticsOpen && (
                <div className="border-t border-[var(--border)] px-5 py-4">
                  ...
                  <pre className="max-h-[360px] overflow-auto rounded-lg bg-[#0f0f0f] p-4 font-mono text-[12px] leading-6 text-[#777] whitespace-pre-wrap break-words dark:bg-[#0a0a0a]">
                    {testRunning === service || logs ? logs : t('Waiting for test run...')}
                  </pre>
                </div>
              )}
            </div>
...
function ContextWindowDetectionBanner({
  ...
  const currentRaw = Number.parseInt(String(model.context_window || '').replace(/[^\d]/g, ''), 10)
  const matches = Number.isFinite(currentRaw) && currentRaw === detection.contextWindow
  ...
  return (
    <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-2 rounded-lg border border-[var(--border)] bg-[var(--muted)]/30 px-3 py-2 sm:col-span-2">
      <div className="flex min-w-0 flex-1 flex-wrap items-baseline gap-x-2 gap-y-0.5 text-[12px]">
        ...
        {detectedAt && <span className="text-[var(--muted-foreground)]/60">· {detectedAt}</span>}
      </div>
      <button
        type="button"
        onClick={onApply}
