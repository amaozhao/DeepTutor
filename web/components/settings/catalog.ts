export type ServiceName = 'llm' | 'embedding' | 'search' | 'tts' | 'stt' | 'imagegen' | 'videogen'
...
export type ServiceReadiness = 'not_configured' | 'untested' | 'passed' | 'failed'
...
export function getActiveModel(catalog: Catalog, serviceName: ServiceName): CatalogModel | null {
  if (serviceName === 'search') return null
  const service = catalog.services[serviceName]
  const profile = getActiveProfile(catalog, serviceName)
  if (!profile) return null
  return (
    profile.models.find(model => model.id === service.active_model_id) ?? profile.models[0] ?? null
  )
}

export function serviceConfigured(catalog: Catalog, serviceName: ServiceName): boolean {
...
export function currentDiagnosticsResult(
  ...
  const modelId = serviceName === 'search' ? null : (service.active_model_id ?? null)
  return diagnostics.profileId === profileId && diagnostics.modelId === modelId ? diagnostics : null
}
...
export function serviceReadiness(
  ...
  const diagnostics = currentDiagnosticsResult(catalog, serviceName, diagnosticsResults)
...
export function servicePendingApply(
  ...
  return JSON.stringify(catalog.services[service]) !== JSON.stringify(draft.services[service])
}

export function nextModelName(models: CatalogModel[], language: UiSettings['language']): string {
